// Browser-compatible Package Search UI Component
class PackageSearchUI {
    constructor() {
        this.searchTimeout = null;
        this.searchInput = document.getElementById('package-search-input');
    ...
    async performSearch(query) {
        ...
        this.searchResults.querySelectorAll('.install-package-btn').forEach(button => {
            button.addEventListener('click', (event) => {
                this.handleInstall(event.target);
            });
        });

        // Check if results container is scrollable
    ...
    updateStatus(message, isError) {
        ...
        this.statusElement.classList.remove('bg-green-50', 'border-green-200', 'text-green-800', 'bg-red-50', 'border-red-200', 'text-red-800', 'bg-blue-50', 'border-blue-200', 'text-blue-800');
        if (isError) {
...
window.PackageSearchUI = PackageSearchUI;
