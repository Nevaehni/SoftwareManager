...
function setupIpcHandlers(): void {    // Backup packages
    ...
    ipcMain.handle('download-custom-installer', async (event, url: string) => {
        try {
            ...
            console.error('Error downloading custom installer:', error); return {
                success: false,
                error: error instanceof Error ? error.message : 'Unknown error'
            };
        }
    ...
    ipcMain.handle('load-spec-file', async () => {
        try {
            const result = await dialog.showOpenDialog(mainWindow!, {
    ...
    ipcMain.handle('save-spec-file', async (event, content: string, language: string) => {
        try {
    ...
    ipcMain.handle('validate-spec', async (event, content: string, language: string) => {
        try {
            const errors: string[] = [];
            ...
            if (parsedData.packages) {
                if (!Array.isArray(parsedData.packages)) {
                    ...
                    parsedData.packages.forEach((pkg: any, index: number) => {
            ...
            if (parsedData.customInstallers) {
                if (!Array.isArray(parsedData.customInstallers)) {
                    ...
                    parsedData.customInstallers.forEach((installer: any, index: number) => {
